...
请报告影响[当前 Stable 通道](https://github.com/kuasar-sandbox/kuasar-sandbox/releases/latest)、最新已发布 Aggregate Preview 或当前 `main` 的漏洞,并提供实际受影响的精确聚合版本和组件版本。较早 Preview 中的问题会根据是否仍影响当前 Stable、最新 Preview 或 `main` 进行评估。

| 版本范围 | 当前状态 |
| --- | --- |
| GitHub Latest 指向的非 prerelease 聚合版本 | 当前 Stable 支持版本 |
| 最新已发布 Aggregate Preview | 接受预发布通道问题报告 |
| 当前 `main` | 接受源码问题报告 |

Preview 是 GitHub prerelease；系统是否支持生产部署与公开发行通道的稳定性标记是两个不同维度。

本文件定义漏洞接收、协调和披露政策,不替代实现安全模型.系统信任边界见
[多租安全](docs/kuasar-sandbox_zh.md#73-多租安全);各组件链接的设计文档仍分别拥有协议、
存储、Runtime 和网络机制规范.
