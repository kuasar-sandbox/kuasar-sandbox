...
实现留在所属组件内部,跨仓只导出具体包.上图描述仓库级关系,不是完整的包依赖白名单:
orchestrator 使用 sandboxer 的资源和工件/恢复 API,MMDS source 索引还使用 connector
vSwitch 常量.实际依赖集合由当前 module 文件与 import 确定.五个组件既可组合为完整平台,
也可按场景独立采用.
...
捕获内存的暂停保留同一个逻辑实例的进程、内存和文件系统状态;显式 pause 默认采用
这种快照行为.指定 `memory: false` 时暂停保存磁盘工件,之后重新启动应用,不恢复先前
进程内存.`autoPauseMemory` 独立选择自动暂停时的捕获行为.两种情况下,连接/恢复都
继续使用稳定的公开 Sandbox ID.工件保留在原节点时可以原地恢复;工件发布到命名共享文件位置或 Manifest
...
[Stable 通道](https://github.com/kuasar-sandbox/kuasar-sandbox/releases/latest)解析为
当前非 prerelease 聚合版本.一次安装只解析一次,随后对全部资产和校验和使用同一精确
Tag.Preview 继续用于开发和评估.当前 GitHub Release 提供 Linux x86_64 预构建资产;
