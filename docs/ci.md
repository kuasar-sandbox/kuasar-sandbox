...
Preparation supplies manifest Docker archives, the guest flatten fixture, resolved container image IDs/digests, orchestrator base-image fixtures and target helper binaries (zot, versitygw, custom Proxy, telemetry probe, sandboxer usage probe, orchestrator CLI tests). The orchestrator profile compiles `orch-cli.test` from its exact test pin during the existing helper build and supplies `ORCH_CLI_TEST_BIN` to the owner runner; its hash, target architecture and test revision pass the same preparation/provenance checks as other helpers. It consumes the independently selected `BIN` products. The prepared workspace contains `bin/`, `test/`, `fixtures/`, `images/`, license/material files and `provenance.json`. Helpers/fixtures are preparation inputs; business Build, flatten, snapshot, publish and restore operations remain in their original E2E cases.
