...
`platform` 这个 umbrella 目录承载。本文是系统设计总览:
...
| 仓 | 角色 | 关键进程/产物 | 导出面 | 详设 |
|---|---|---|---|---|
| **platform** | 系统文档 + 发布聚合 + E2E 统一环境/owner 套件组装 + platform 组合用例/perf | `release-suite.sh` 顺序发布独立组件与聚合版本;源码树 `make e2e-tools` 可辅助获取测试环境工具 | — | 本文 + `deployment.md`/`release.md`/`perf.md` |
