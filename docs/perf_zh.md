...
`PHASES=all|calibrate|pause_resume|cold|ramp|stress|self_check` 选择阶段.

生成报告中的每个声明 target 都以本次运行自身的测量值为比较对象,计算并渲染明确的
PASS/FAIL;执行成功从不替代性能成功,生成报告时也不会为通过而调高/删除/重释阈值.
每个 target 按"结果证据合同"(下文 §2)标注为 gate 或 informational target:gate
未达标时 harness 以非零退出,失败验证不会记成绿色运行;informational target 如实
报告未达标但不决定退出状态.`PHASES=self_check`(`make perf-agent-selfcheck`)回放
固定报告 fixtures,包含一个刻意超标的 restore 样本;若报告机制可能把超标样本渲染成
成功证据则该回归直接失败.该回归同时接入 `make test-perf-tools`.
...
容量规划除应用 RSS 外,还必须包含 guest 内核、页表、slab 和 guest 服务占用,并按
workload 实测预留余量.guest 内 OOM 与 host cgroup OOM 是不同事件:host
`memory.events` 的 OOM 计数为零不能证明 guest 应用存活.还必须记录应用完成、退出及
guest 侧证据.退出码 137 表示 SIGKILL 类退出,单凭此值不能判定其原因.本文不设定
普遍 guest 开销常数或固定 capacity 倍率.
...
- [kuasar-sandbox_zh.md](kuasar-sandbox_zh.md) - 系统语义,组件边界与性能证据要求
- [`deployment_zh.md`](deployment_zh.md) - 数据后端,进程拓扑和部署选择
- [accelerator Cache](https://github.com/kuasar-sandbox/accelerator/blob/main/docs/cache_zh.md)(发布包:`docs/cache_zh.md`) - cache 架构和组件 benchmark
- [sandboxer 生命周期](https://github.com/kuasar-sandbox/sandboxer/blob/main/docs/sandbox_zh.md)(发布包:`docs/sandbox_zh.md`) - snapshot/restore 和统计字段
- [节点资源](https://github.com/kuasar-sandbox/orchestrator/blob/main/docs/node-resource_zh.md)(发布包:`docs/node-resource_zh.md`) - 节点资源控制协议
- [集群设计](https://github.com/kuasar-sandbox/orchestrator/blob/main/docs/cluster_zh.md)(发布包:`docs/cluster_zh.md`) - registry/router/placer 设计
