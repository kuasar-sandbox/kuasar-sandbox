...
默认基础镜像是按 digest 固定的 Docker Official `python:3.12-slim` linux/amd64 manifest;其 Python Runtime 足以完成构建、就绪、执行和数据访问检查。如需使用已有 Registry 而不是 Demo 所属 Zot,传入 `REGISTRY=<host[:port]>`。需要认证时传入 `REGISTRY_USER` 与 `REGISTRY_PASS`;`demo_prep.sh` 通过 stdin 向 `docker login` 传密码,Docker 认证只存放在私有 Demo 数据目录。只有明确使用 HTTP Registry 时才设置 `REGISTRY_INSECURE=1`。覆盖 `E2E_IMAGE` 时必须使用不可变的 `name@sha256:<digest>` 引用。
