...
class DemoStateAssertions(unittest.TestCase):
    def exercise(self, phase, *, listed=True, status=0, state=None, data=None):
        ...
        values = types.SimpleNamespace(killed=False, migration=False)

        class Sandbox:
            @staticmethod
            def list():
                return types.SimpleNamespace(next_items=lambda: [types.SimpleNamespace(sandbox_id="fixture-child")] if listed else [])
            ...
            def connect(identifier, **options):
                ...
                command = types.SimpleNamespace(run=lambda _: types.SimpleNamespace(
                    exit_code=status, stderr="fixture", stdout="hello from before the snapshot" if state is None else state))
