...
from artifacts import case_name, profiles
...
# Reuse the ordinary owner profile, including the opt-in OBS exclusion.
profile = profiles([owner], "x86_64", {owner: [case.name for case in cases]})
selected = [Path(case).name for case in profile["cases"]]
if not selected:
    raise SystemExit(f"{owner} source has no default E2E cases")
...
script += ''.join(' --include ' + shlex.quote(case) for case in selected) + '\n'
