...
if ! docker image inspect openclaw-agent:latest >/dev/null 2>&1; then
    echo "==> Building OpenClaw agent container image openclaw-agent:latest..."
    docker build -t openclaw-agent:latest "$REPO_ROOT/test/perf/workloads/openclaw-image"
fi
IMAGE_ID=$(docker image inspect openclaw-agent:latest --format '{{.Id}}' 2>/dev/null || true)
if [ -z "$IMAGE_ID" ] || [ ! -f "$BLK0" ] || [ "$(cat "$BLK0_ID" 2>/dev/null)" != "$IMAGE_ID" ]; then
...
RAW_DIR="$OUT_BASE/raw-logs-$(date +%Y%m%d-%H%M%S)"

cleanup() {
    set +e
    echo; echo "==> Soak cleanup..."
    mkdir -p "$RAW_DIR"; cp -a "$WORK"/*.log "$WORK"/*.csv "$WORK"/*.yaml "$RAW_DIR"/ 2>/dev/null || true
...
audit() {
    local audit_file="$RAW_DIR/audit.txt"
    {
        ...
        echo "  guest SIGKILL events:   $(grep -l 'signal=9\|code=137' "$RAW_DIR"/oc-*.log 2>/dev/null | wc -l)"
        echo "  admission rejections:   $(grep -l 'admit rejected' "$RAW_DIR"/oc-*.log 2>/dev/null | wc -l)"
        echo "  uffd handler errors:    $(grep -h 'errors  *[1-9]' "$RAW_DIR"/oc-*.log 2>/dev/null | wc -l)"
        echo "  CH nonzero exits:       $(grep -h 'CH exited code=[^0]' "$RAW_DIR"/oc-*.log 2>/dev/null | wc -l)"
        echo "  CSV: $CSV"
        echo "  Logs: $RAW_DIR"
    } | tee "$audit_file"
}
...
EOF
        ...
        unset rest_pids
        for k in 1 2 3 4; do
            ip link delete "tapr${WAVE}-${k}" 2>/dev/null || true
            rmdir "/sys/fs/cgroup/sandboxes/oc-w${WAVE}-rest-${k}" 2>/dev/null || true
        done
    ...
    # Active wave drain: as soon as a sandbox logs "Verdict: PASS" (or fails),
    # send SIGTERM to its sandbox-ctl process so it shuts down CH cleanly
    ...
    while :; do
        ...
        for idx in "${!local_pids[@]}"; do
            ...
            if kill -0 "$p" 2>/dev/null; then
                alive=$((alive+1))
                if [ -z "${term_sent[$sid]:-}" ]; then
                    if grep -q "Verdict: PASS\|AgentSession Fatal Error\|CH exited code\|signal=9\|code=137" "$WORK/$sid.log" 2>/dev/null; then
