...
.PHONY: all build collect e2e-zot e2e-versitygw e2e-tools assemble-e2e release verify-prebuilt vet test test-ci-tools test-release-tools test-perf-tools test-uffd-performance-gate clean help demo \
	        bench e2e-fixtures test-e2e test-e2e-prebuilt perf dedup-report perf-agent perf-agent-soak \
	        $(PERF_TARGETS)
...
# Long-running mixed-fleet soak on the same workload. SOAK_DURATION in seconds.
SOAK_DURATION ?= 1200
perf-agent-soak: build
	BIN=$(SBIN) bash test/perf/openclaw-soak.sh $(SOAK_DURATION)
...
help:
	...
	@echo "  perf-agent-soak  mixed-fleet sustainment soak (SOAK_DURATION seconds, default 1200)"
